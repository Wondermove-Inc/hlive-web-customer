...
import { SOCKET_URI, SOCKET_NAMESPACE, WEBRTC_URI, WEBRTC_SLAVE_URI, WEBRTC_PORT, WEBRTC_ROOM_URI, WEBRTC_PATH } from '@constants';
...
const useRtc = ({ chatRoomId, dealerYn }) => {
  console.log('look at this!!', chatRoomId, dealerYn);
  ...
  const [peerErrorMessage, setPeerErrorMessage] = useState<string>();
  ...
  const [customerMicOnYn, setCustomerMicOnYn] = useState<boolean>(false);
  ...
  // const userType = dealerYn !== 'CUSTOMER' ? 'DEALER' : 'CUSTOMER';
  ...
  const findDestination = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(`${WEBRTC_ROOM_URI}/peers/room`, {
        params: {
          roomId: chatRoomId,
          userType: userType,
          peerId: peerId,
      ...
      // console.log('🚀 ~ file: useRtc.ts:471 ~ findDestination ~ response', response);
  ...
  const connect = useCallback(() => {
    ...
    setConnecting(true);
    try {
      const conn = local.connect(destination);
      conn.on('data', (data) => {
        setConnected(true);
        setConnecting(false);
  ...
  const socketInitializer = async (_id) => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('microphone', { roomId: _id, sender: 'DEALER', onYn: micOnYn });
      socket.emit('camera', { roomId: _id, sender: 'DEALER', onYn: cameraOnYn });
    });
  ...
  useEffect(() => {
    if (remoteStream) {
      ...
      if (remoteStream.getVideoTracks().length > 0) {
        ...
        remoteStream?.getVideoTracks()[0]?.addEventListener('mute', function (e) {
          console.log('remote camera muted!');
          setStreamCameraErrored(true);
          // setCustomerCameraOnYn(false);
          // setConnectError(true);
        });
        remoteStream?.getVideoTracks()[0]?.addEventListener('unmute', function (e) {
      ...
      if (remoteStream.getAudioTracks().length > 0) {
        remoteStream.getAudioTracks()[0]?.addEventListener('mute', function (e) {
          console.log('remote mic muted!');
          setStreamMicErrored(true);
          // setCustomerMicOnYn(false);
          // setConnectError(true);
        });
        remoteStream.getAudioTracks()[0]?.addEventListener('unmute', function (e) {
  ...
  useEffect(() => {
    ...
    if (remotePlayerRef.current && remoteStream) remotePlayerRef.current.srcObject = remoteStream ? remoteStream : null;

    return () => {
      if (remotePlayerRef.current) {
        (remotePlayerRef.current.srcObject as MediaStream)?.getTracks().forEach((track) => {
          track.stop();
        });
  ...
  useEffect(() => {
    ...
    if (playerRef.current && localStream) playerRef.current.srcObject = localStream ? localStream : null;

    return () => {
      if (playerRef.current) {
        (playerRef.current.srcObject as MediaStream)?.getTracks().forEach((track) => {
          track.stop();
        });
  ...
  useEffect(() => {
    if (networkErrored && socketInstance) {
      socketInstance?.emit('consultError', { consultId: chatRoomId, sender: 'DEALER' });
    }
  }, [networkErrored]);
  // // 얜 뭐지
  // useEffect(() => {
